  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) {
        ...
        return MOCK_LEADS // Retorna dados mockados em caso de erro
      }

      // Se tiver poucos dados reais, combina com mockados
      if (!realData || realData.length < 3) {
        const combinedData = [...(realData || []), ...MOCK_LEADS]
        return combinedData
      }
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) {
        ...
        return MOCK_LEADS.filter(lead => (lead.lead_score || 0) >= minScore)
      }

      // Se tiver poucos dados reais, combina com mockados
      if (!realData || realData.length < 2) {
        const mockFiltered = MOCK_LEADS.filter(lead => (lead.lead_score || 0) >= minScore)
        const combinedData = [...(realData || []), ...mockFiltered]
        return combinedData
      }
